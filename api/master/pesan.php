...
{
  $outp   = [];
  $proses = $conn->query("SELECT tbl_pesan_khusus.*, (tbl_user.nama) AS pengirim FROM tbl_pesan_khusus INNER JOIN tbl_user ON tbl_pesan_khusus.dari=tbl_user.id");
  if ($proses->num_rows > 0) {
    while($rs = $proses->fetch_object()) {
...
{
  $outp   = [];
  $proses = $conn->query("SELECT tbl_pesan_broadcast.*, (tbl_user.nama) AS pengirim FROM tbl_pesan_broadcast INNER JOIN tbl_user ON tbl_pesan_broadcast.dari=tbl_user.id");
  if ($proses->num_rows > 0) {
    while($rs = $proses->fetch_object()) {
      if ($rs->kepada != '') {
